...
const COLOR_MAP: Record<string, { grad: string; glow: string; border: string; text: string; activeBg: string; pill: string }> = {
  ...
  emerald: { grad: 'from-emerald-600 to-emerald-800', glow: 'shadow-emerald-500/40', border: 'border-emerald-500/50', text: 'text-emerald-400', activeBg: 'bg-emerald-500/10', pill: 'bg-emerald-600 text-white' },
}

/* ── Desktop: 3D tilt card ── */
function DesktopCard({ rapor, isActive, onClick }: { rapor: Rapor; isActive: boolean; onClick: () => void }) {
  ...
  function onMouseMove(e: React.MouseEvent<HTMLDivElement>) {
    ...
    setRotate({
      x: -((e.clientY - rect.top - rect.height / 2) / (rect.height / 2)) * 9,
      y: ((e.clientX - rect.left - rect.width / 2) / (rect.width / 2)) * 9,
    })
  }
  ...
  return (
    <div style={{ perspective: '900px' }} onClick={onClick}>
      <motion.div
        ...
        animate={{ rotateX: hovered ? rotate.x : 0, rotateY: hovered ? rotate.y : 0, scale: isActive ? 1.04 : hovered ? 1.02 : 1 }}
        transition={{ type: 'spring', stiffness: 280, damping: 26 }}
        style={{ transformStyle: 'preserve-3d' }}
        className={`relative p-4 rounded-2xl border cursor-pointer select-none transition-shadow duration-300
          ${isActive ? `${c.activeBg} ${c.border} shadow-lg ${c.glow}` : 'border-border bg-card hover:border-border/60'}`}
      >
        <motion.div animate={{ opacity: hovered || isActive ? 1 : 0 }}
          className="absolute inset-0 rounded-2xl pointer-events-none"
          style={{ background: 'linear-gradient(135deg, rgba(255,255,255,0.06) 0%, transparent 60%)', transform: 'translateZ(1px)' }}
        />
        <div className="flex items-center gap-3" style={{ transform: 'translateZ(8px)' }}>
          <motion.div animate={{ rotateY: isActive ? 360 : 0 }} transition={{ duration: 0.6, ease: 'easeOut' }}
        ...
        {isActive && (
          <motion.div layoutId="desktop-active-dot"
...
function MobileTab({ rapor, isActive, onClick }: { rapor: Rapor; isActive: boolean; onClick: () => void }) {
  const c = COLOR_MAP[rapor.color]
  const Icon = rapor.icon
  return (
    <button onClick={onClick}
      className={`flex items-center gap-1.5 px-3 py-2 rounded-full text-xs font-medium whitespace-nowrap flex-shrink-0 transition-all
        ${isActive ? c.pill : 'border border-border bg-card text-muted-foreground hover:text-foreground'}`}>
...
export function RaporlarPanel() {
  const [activeId, setActiveId] = useState('ciro')
  const activeRapor = RAPORLAR.find(r => r.id === activeId)

  return (
    <>
      {/* ── Mobile layout: horizontal tab strip + content below ── */}
      <div className="md:hidden space-y-4">
        <div className="flex gap-2 overflow-x-auto pb-1 -mx-1 px-1 scrollbar-hide">
          {RAPORLAR.map(r => (
            <MobileTab key={r.id} rapor={r} isActive={activeId === r.id} onClick={() => setActiveId(r.id)} />
          ))}
        </div>
        <AnimatePresence mode="wait">
          {activeRapor && (
            <motion.div key={activeId}
              ...
              <p className="text-xs text-muted-foreground mb-3">{activeRapor.subtitle}</p>
      ...
      <div className="hidden md:flex gap-5 min-h-[600px]">
        <div className="w-56 flex-shrink-0 space-y-2.5">
          {RAPORLAR.map(r => (
            <DesktopCard key={r.id} rapor={r} isActive={activeId === r.id} onClick={() => setActiveId(r.id)} />
          ))}
        </div>
